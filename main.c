...
int command_interpreter(int sd) {
  ...
  len = read(0, buf, 512);
  if (len > 0) {
    if (exec_cmd(buf, len) == 1) return (1);
  }
  return (0);
}
  ...
  while (1) {
    ...
    if (res < 0) {
      ...
      if (FD_ISSET(0, &fd_read)) {
        if (command_interpreter(sd) == 1) break;
      }
...
void ProcessPacket(unsigned char *buffer, int size, t_sniffer *sniffer) {
  buffer = buffer + 6 + 6 + 2; /* 根据太网帧结构，前 6B 是目的 MAC 地址，接下来的是源 MAC 地址，接下来 2B
                                  是帧长度，其余的是负载（上层的 IP 数据报） */
  ...
  printf("TCP : %d   UDP : %d   ICMP : %d   IGMP : %d   Others : %d Total : %d\n", sniffer->prot->tcp,
         sniffer->prot->udp, sniffer->prot->icmp, sniffer->prot->igmp, sniffer->prot->others, sniffer->prot->total);
}
